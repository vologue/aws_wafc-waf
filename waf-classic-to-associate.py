...
for acl in val['WebACLs']:
    ...
    rulematch.region = acl_desc["WebACLArn"].split(':')[-3]
    rules = list()
    ...
    command  = "aws wafv2 create-web-acl --name %s --scope REGIONAL --default-action %s={} --rules file://rulegroup.json --visibility-config %s --region %s" %(name,def_action,visibilityconfig,rulematch.region)
    print(command)
